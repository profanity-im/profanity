...
void
chatwin_incoming_msg(ProfChatWin *chatwin, const char *const resource, const char *const id,
    const char *const correct_id, const char *const message, GDateTime *timestamp, gboolean win_created,
    prof_enc_t enc_mode)
{
    assert(chatwin != NULL);
    ...
    gboolean request_receipt, char *correct_id)
{
    ...
    win_print_outgoing((ProfWin*)chatwin, enc_char, message, id, request_receipt);
}
...
chatwin_outgoing_carbon(ProfChatWin *chatwin, const char *const message, prof_enc_t enc_mode)
{
    ...
    win_print_outgoing((ProfWin*)chatwin, enc_char, message, NULL, FALSE);
