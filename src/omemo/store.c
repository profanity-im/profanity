...
#endif
{
    ...
    log_info("[OMEMO][STORE] Looking for %s in session_store", address->name);
    ...
    log_info("[OMEMO][STORE] Looking for device %d of %s ", address->device_id, address->name);
...
#endif
{
    ...
    log_info("[OMEMO][STORE] Store session for %s (%d)", address->name, address->device_id);
...
remove_pre_key(uint32_t pre_key_id, void* user_data)
{
    ...
    if (ret > 0) {
        ...
        log_info("[OMEMO][STORE] SG_ERR_INVALID_KEY_ID");
        return SG_ERR_INVALID_KEY_ID;
    }
}
              ...
              size_t key_len, void* user_data)
{
    ...
    if (identity_key_store->recv) {
        ...
        if (trusted == 0) {
            log_info("[OMEMO][STORE] trusted 0");
                    ...
                    size_t key_len, void* user_data)
{
    int ret;
    identity_key_store_t* identity_key_store = (identity_key_store_t*)user_data;
    log_info("[OMEMO][STORE] Checking trust %s (%d)", address->name, address->device_id);
    GHashTable* trusted = g_hash_table_lookup(identity_key_store->trusted, address->name);
    if (!trusted) {
        if (identity_key_store->recv) {
            log_info("[OMEMO][STORE] identity_key_store->recv");
            return 1;
        } else {
            log_info("[OMEMO][STORE] !identity_key_store->recv");
            return 0;
    ...
    if(!original) {
        log_info("[OMEMO][STORE] original not found %s (%d)", address->name, address->device_id);
    }
    ...
    if (identity_key_store->recv) {
        log_info("[OMEMO][STORE] 1 identity_key_store->recv");
        return 1;
    } else {
        log_info("[OMEMO][STORE] Checking trust %s (%d): %d", address->name, address->device_id, ret);
        return ret;
    }
}
